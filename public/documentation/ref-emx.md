...
Defines the data type (default: string)
  ...
  * one_to_many (MOLGENIS 2.0): A data type that defines the one to many relationship between two columns in two separate tables. Example: An author wrote one or more books. An book is written by one author.
### refEntity 
